// Static configuration for subscription tiers and limits
// This file is for use in server components and metadata generation

export const getStaticSubscriptionConfig = () => {
  const config = {
...
export const getStaticTrialPeriodText = (): string => {
...
export const getStaticTrialPeriodFullText = (): string => {
  const config = getStaticSubscriptionConfig()
  if (config.trialDays === 0) {
    return ''
  } else if (config.trialDays === 1) {
...
export const isStaticTrialEnabled = (): boolean => {
  const config = getStaticSubscriptionConfig()
